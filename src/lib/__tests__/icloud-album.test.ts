...
describe('fetchSharedStreamsAlbum', () => {
  it('preserves album-add timestamps, falls back to capture dates, and keeps undated items', async () => {
    routeFetch({ webstream: () => webstreamResponse([
      { ...photoItem('added'), batchDateCreated: '2026-09-01T15:30:00Z', dateCreated: '2026-09-01T13:47:00Z' },
      { ...photoItem('fallback'), batchDateCreated: 'invalid', dateCreated: '2026-08-31T12:00:00Z' },
      { ...photoItem('undated'), batchDateCreated: '', dateCreated: 'invalid' },
      { ...videoItem('video'), batchDateCreated: '2026-09-02T12:00:00Z' },
    ]) });

    const items = await fetchSharedStreamsAlbum('B125ON9t3mbLNC');
    expect(items).toHaveLength(4);
    expect(items[0].addedAt).toBe(Date.parse('2026-09-01T15:30:00Z'));
    expect(items[1].addedAt).toBe(Date.parse('2026-08-31T12:00:00Z'));
    expect(items[2]).not.toHaveProperty('addedAt');
    expect(items[3].addedAt).toBe(Date.parse('2026-09-02T12:00:00Z'));
  });
