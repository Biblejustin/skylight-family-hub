...
const WEBASSETURLS_CONCURRENCY = 4;
...
async function postSharedStreams(url: string, body: unknown): Promise<Response> {
  return fetchWithTimeout(url, {
    ...
    // Large public albums can take over 30 seconds just to return metadata.
    timeout: url.endsWith('/webstream') ? 60_000 : 15_000,
  });
}
...
async function fetchAssetUrls(token: string, photoGuids: string[]): Promise<Map<string, string>> {
  ...
  // Bound parallel requests: a large family album may need 90+ batches.
  for (let i = 0; i < photoGuids.length; i += WEBASSETURLS_BATCH_SIZE * WEBASSETURLS_CONCURRENCY) {
    await Promise.all(Array.from({ length: WEBASSETURLS_CONCURRENCY }, async (_, offset) => {
      const start = i + offset * WEBASSETURLS_BATCH_SIZE;
      const batch = photoGuids.slice(start, start + WEBASSETURLS_BATCH_SIZE);
...
/** Prefer when the item joined the shared album over when it was captured. */
function albumAddedAt(photo: RawPhoto): number | undefined {
  for (const value of [photo.batchDateCreated, photo.dateCreated]) {
...
async function resolveAlbum(token: string): Promise<ICloudAlbumItem[]> {
  ...
  for (const photo of photos) {
    ...
    const addedAt = albumAddedAt(photo);
    const dateMetadata = addedAt === undefined ? {} : { addedAt };
    if (photo.mediaAssetType === 'video') {
      ...
      plans.push({ guid: photo.photoGuid, type: 'image', mainChecksum: image.checksum, ...dateMetadata });
    }
  }
  ...
  for (const plan of plans) {
    ...
    if (!url) continue;
    const dateMetadata = plan.addedAt === undefined ? {} : { addedAt: plan.addedAt };
    if (plan.type === 'video') {
      const posterUrl = plan.posterChecksum ? urls.get(plan.posterChecksum) : undefined;
      items.push({ url, type: 'video', guid: plan.guid, ...(posterUrl ? { posterUrl } : {}), ...dateMetadata });
    } else {
      items.push({ url, type: 'image', guid: plan.guid, ...dateMetadata });
    }
  }
