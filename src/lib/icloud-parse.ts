 ...
 * or accept a bare legacy token. Extended D tokens require an explicit album
 * URL so bare URL-safe iCloud Link GUIDs keep their existing meaning.
...
export function parseICloudAlbumToken(input: string): string | null {
  ...
  const candidate = (hashIndex >= 0 ? trimmed.slice(hashIndex + 1) : trimmed)
    ...
    .split(';')[0]
    .trim();
  if (/^[A-Za-z0-9]{6,128}$/.test(candidate)) return candidate;

  // Apple's sharedalbum web client accepts D alongside A/B/C and still reads
  // the partition from the next two base62 characters. The remaining D token
  // can be long and URL-safe base64; keep it intact for sharedstreams.
  if (!/^D[A-Za-z0-9]{2}[A-Za-z0-9_-]{3,4093}$/.test(candidate)) return null;
  try {
    const url = new URL(trimmed.startsWith('http') ? trimmed : `https://${trimmed}`);
    const isAlbumUrl = /^https?:$/.test(url.protocol)
      && /(^|\.)icloud\.com$/i.test(url.hostname)
      && /^\/sharedalbum(?:\/[A-Za-z-]+)?\/?$/.test(url.pathname)
      && !!url.hash;
    return isAlbumUrl ? candidate : null;
  } catch {
    return null;
  }
}
