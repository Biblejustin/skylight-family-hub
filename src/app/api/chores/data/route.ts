...
export const PUT = withAuth(async (request: NextRequest) => {
  ...
  if (chores.some((chore) => chore?.frequency === 'interval'
    && !isValidChoreInterval(chore.intervalDays, chore.anchorDate))) {
    return NextResponse.json(
      { error: 'Repeating chores need a positive whole number of days and a valid first date.' },
      { status: 400 },
    );
  }
