import type { ChoreDefinition, ChoreRotation, ChoreResetFrequency } from '@/types/config';
import type { TranslateFn } from '@/i18n';
import { getTimeOfDayLabelKey } from './types';
import { isValidChoreInterval } from '@/lib/chore-interval';
...
export function getChoreValidationHintKind(args: {
  ...
  if (args.frequency === 'interval' && !isValidChoreInterval(args.intervalDays, args.anchorDate)) {
...
export function buildChoreSummaryLine(args: {
  ...
  else if (chore.frequency === 'interval') {
    frequencyLabel = t('chore-chart.choreSummary.interval', { count: chore.intervalDays ?? 0, date: chore.anchorDate ?? '' });
  }
