...
export function useChoreForm(
  ...
  const [intervalDays, setIntervalDays] = useState(initial?.intervalDays?.toString() ?? '3');
  ...
  const submit = (onSubmit: (data: Omit<ChoreDefinition, 'id'>) => void) => {
    ...
    onSubmit({
      ...
      frequency,
      daysOfWeek: frequency === 'interval' ? [] : finalDaysOfWeek,
      timeOfDay,
      ...
      ...(frequency === 'interval' ? { intervalDays: Number(intervalDays), anchorDate } : {}),
    });
  };

  return {
    name, emoji, points, frequency, daysOfWeek, specificDate, intervalDays, anchorDate, timeOfDay,
    assigneeIds, rotation, schedule,
    setName, setEmoji, setPoints, setFrequency, setSpecificDate, setIntervalDays, setAnchorDate, setTimeOfDay,
...
export function useChoreLabelMaps(t: TranslateFn): ChoreLabelMaps {
  const frequencyLabelMap = useMemo<Record<ChoreResetFrequency, string>>(
    () => ({
      ...
      interval: t('chore-chart.frequency.interval'),
