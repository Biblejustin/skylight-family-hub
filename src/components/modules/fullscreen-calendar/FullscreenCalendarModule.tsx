...
export interface FullscreenCalendarModuleProps {
...
export default function FullscreenCalendarModule({
  ...
  const headerTitle = traits.headerTitle({ today: displayedDate, t, locale, config, scaleWidth: scale.width });
  ...
  const legend = useMemo(() => {
    ...
    const window = traits.legendWindow({ today: displayedDate, weekStartsOn: weekStartsOnFor(config.startDay), config, scaleWidth: scale.width });
    return buildLegend(events, window, timezone, t('calendar.publicHolidays'));
  }, [legendPlacement, events, timezone, displayedDate, t, traits, config, scale.width]);
  ...
  return (
    ...
    >
      ...
      <header className="fsc-header" style={{ height: `${Math.max(scale.bu * 5 * Math.max(1, scale.typoMul), hasNavigation ? 56 : 0)}px`, flexShrink: 0 }} role="banner">
        <h1
          ...
          style={{
            fontSize: `${scale.bu * 3.5 * scale.typoMul}px`, lineHeight: 1.15, margin: 0,
            ...(hasNavigation ? { flex: '1 1 auto', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis' } : {}),
          }}
        >
          {headerTitle}
        </h1>
        {!hasNavigation && <div style={{ flex: 1 }} />}
        {status.kind === 'badge' ? (
        ...
        {hasNavigation ? (
          <div style={{ display: 'flex', alignItems: 'center', flexShrink: 0, minHeight: 44 }}>
            {navigation}
          </div>
        ) : <span
      ...
      <div className="fsc-content" style={{ flex: 1, minHeight: 0 }}>
        ...
        ) : neverLoaded || (!hasContent && !showEmptyGrid) ? (
