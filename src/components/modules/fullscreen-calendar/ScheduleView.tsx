...
export function ScheduleView({ events, timezone, config, scale, today, now, viewDate, timeFormat = DEFAULT_TIME_FORMAT, weather }: CalendarViewProps) {
  ...
  const scheduleStart = useMemo(
    () => resolveScheduleStart(displayedDate, config.scheduleStartAnchor, weekStartsOn),
    [displayedDate, config.scheduleStartAnchor, weekStartsOn],
  );
