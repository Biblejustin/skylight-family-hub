...
export function MonthGridView({ events, timezone, config, scale, today, now, viewDate }: CalendarViewProps) {
  ...
  const { cells, weekCount } = useMemo(() => {
    ...
    const monthEnd = endOfMonth(displayedDate);
    ...
    // The browsed date is stable across real-clock ticks.
  }, [displayedDate, weekStartsOn]);

  // Localized day-of-week labels derived from the active formatting locale
  const dowDates = useMemo(
    () => Array.from({ length: 7 }, (_, i) => addDays(cells[0] ?? displayedDate, i)),
    [cells, displayedDate],
  );

  return (
    <div role="grid" aria-label={formatDateSync(displayedDate, 'MMMM yyyy', { locale })} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      {/* Day-of-week header */}
      <div role="row" style={{
        display: 'grid',
      ...
      <div ref={scrollRef} style={{
        ...
        {cells.map((day, i) => {
          ...
          const isCurrentMonth = isSameMonth(day, displayedDate);
