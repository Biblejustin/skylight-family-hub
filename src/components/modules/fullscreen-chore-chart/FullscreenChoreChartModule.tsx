...
export default function FullscreenChoreChartModule({
  ...
  const { todayAssignments, memberStats, weekData, members, chores, rewards, recentRedemptions, allRedemptions, toggleComplete, toggleCompleteResult, isLoading, error } = useChoreData(config);
  ...
  const [selectedChoreMemberId, setSelectedChoreMemberId] = useState<string | null>(null);

  const handlePersonToggle = useCallback((choreId: string, memberId: string) => {
    const assignment = todayAssignments.find((item) => item.chore.id === choreId && item.memberId === memberId);
    if (!allowTouch || memberId !== selectedChoreMemberId || !assignment) return Promise.resolve(false);
    // Retrying a failed request must not undo a completion that reached the
    // hub but whose response was lost on Wi-Fi.
    return toggleCompleteResult(choreId, memberId, assignment.isCompleted ? 'uncomplete' : 'complete');
  }, [allowTouch, selectedChoreMemberId, todayAssignments, toggleCompleteResult]);
    ...
    >
      ...
      {effectiveView === 'rewards-store' ? (
        <RewardsStoreView
          ...
          initialMemberId={config.personSelection ? selectedChoreMemberId ?? undefined : undefined}
        />
      ) : config.personSelection ? (
        <PersonChoresView
          members={members}
          memberStats={memberStats}
          assignments={todayAssignments}
          selectedMemberId={selectedChoreMemberId}
          onSelectMember={setSelectedChoreMemberId}
          allowTouch={allowTouch}
          showPoints={config.showPoints}
          showTimeOfDay={config.showTimeOfDay}
          onToggle={handlePersonToggle}
          onRewards={config.showRewardsButton ? () => setShowRewardsOverride(true) : undefined}
          timezone={timezone}
          loading={isLoading}
          error={Boolean(error)}
        />
